import fse from "fs-extra";
import path from "path";
const __dirname = path.resolve();
export async function get_common_data(language) {
 return {} 
}
export async function get_detail_a_data(language) {
 let data = [{page_name:''}] || {page_name:''} || null 
 return data 
}
export async function get_detail_index_data(language) {
 let data = [{page_name:''}] || {page_name:''} || null 
 return data 
}
export async function get_vn_detail_a_data(language) {
 let data = [{page_name:''}] || {page_name:''} || null 
 return data 
}
export async function get_vn_detail_index_data(language) {
 let data = [{page_name:''}] || {page_name:''} || null 
 return data 
}
export async function get_vn_index_data(language) {
 let data = [{page_name:''}] || {page_name:''} || null 
 return data 
}
export async function get_index_data(language) {
 let data = [{page_name:''}] || {page_name:''} || null 
 return data 
}
